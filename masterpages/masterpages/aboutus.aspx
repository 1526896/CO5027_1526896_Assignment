<%@ Page Title="" Language="C#" MasterPageFile="~/Site1.Master" AutoEventWireup="true" CodeBehind="aboutus.aspx.cs" Inherits="masterpages.admin" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
<div class="container2">
<h3>About Us</h3>
<p>It began with a basic arrangement to make a predominant T-shirt. A shirt that gave pressure and underhanded sweat off your skin as opposed to assimilate it. 
    A shirt that worked with your body to manage temperature and improve execution.</p>
<p>Established in 1996 by previous University of Maryland football player Kevin Plank, Under Armour is the originator of execution attire - 
    outfit built to keep competitors cool, dry and light over the span of an amusement, practice or exercise. 
    The innovation behind Under Armour's differing item variety for men, ladies and youth is mind boggling, yet the program for receiving the rewards is basic: wear HeatGear® when it is hot, ColdGear® when it is chilly, and AllSeasonGear® between the extremes.</p>
<p>Under Armour's main goal is to improve all competitors through ethusiasm, plan and the tireless quest for development.</p>
</div>  
</asp:Content>
